...
A linear regression is not appropriate because the variable dvote should only take on the values 0 or 1- not any more or less.
...
```{r}
log_model <- glm(dvote ~ income + gender, family = 'binomial', data = nes)
...
Since the model is logistic, the coefficients cannot be interpreted directly. The average treatment effect can be estimated by dividing the coefficient by four, the "divide by four" rule. The average treatment effect for gendermale is about -0.04, and the effect for income is -0.072. This means that for two voters who are otherwise equal, the probability that the male candidate votes for the Democrat is ~0.04 (about 4%) lower than the probability the female voter votes for the Democrat. The income effect means that for a one-level increase in income bracket, the probability of voting for a Democrat decreases by ~0.072, about 7%.


Bayesian interpretation: 
The probability that the true coefficient is between the bounds of the confidence interval is 0.95. 

Frequentist intepretation:
If this model were run a large number of times, the coefficient estimate would fall within the bounds of the confidence interval 95% of the time.
...
```{r}
augment(log_model, type.predict = 'response')
```
